...
func EnforceCooldown(ctx context.Context, rdb *redis.Client, userID string, action string, duration time.Duration) (bool, time.Duration, error) {
	...
	// Retry loop to handle edge cases where the key expires
	...
	for attempts := 0; attempts < 2; attempts++ {
		...
		// go-redis maps Redis's -2 response to -2 * time.Millisecond
		if pttl == -2*time.Millisecond {
			// Key expired right after our SetArgs failed. Loop around and retry.
			continue
		}

		// go-redis maps Redis's -1 response to -1 * time.Millisecond
		...
		// Catch any sub-zero edge cases (e.g., key expiring in < 1ms)
		if pttl <= 0 {
			continue
		}
