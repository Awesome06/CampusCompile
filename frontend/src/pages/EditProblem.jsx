...
export default function EditProblem() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Fetch Problem Details & All Test Cases
      const loadProblemData = async () => {
        try {
          ...
          // RBAC Protection: Kick out unauthorized users
          ...
          setProblemData({
            ...
            time_limit: p.time_limit_ms,
            memory_limit: p.memory_limit_kb / 1024, // Convert KB back to MB for the UI
            is_public: p.is_public ?? false
          });

          // 2. Fetch ALL test cases for this specific problem
          const tcRes = await api.get(`/problems/${id}/testcases/all`);
          setTestCases(tcRes.data.test_cases || []);
          setIsLoading(false);
  ...
  const handleZipUpload = async (e) => {
    ...
    try {
      ...
      if (newTestCases.length === 0) {
        setStatus({ type: 'error', message: 'No matching input/output files found in ZIP.' });
        return;
      }
          ...
          <div className="w-full max-w-5xl">
                  ...
                  <div className="flex justify-between items-center mb-4 border-b border-dark-border/50 pb-3">
                    ...
                    <div className="flex items-center space-x-5">
                      <label className="flex items-center space-x-2 text-sm text-gray-400 cursor-pointer hover:text-white transition-colors">
                        <input type="checkbox" checked={tc.isHidden ?? tc.is_hidden} onChange={(e) => updateTestCase(index, 'isHidden', e.target.checked)} 
                  ...
                  <div className="flex space-x-6">
                    <div className="w-1/2 flex flex-col">
                      ...
                      <textarea required className="w-full h-40 bg-[#121212] text-gray-300 p-3 rounded border border-dark-border font-mono text-sm outline-none focus:border-dark-accent resize-none custom-scrollbar"
                        value={tc.input || tc.input_data} onChange={(e) => updateTestCase(index, 'input', e.target.value)} />
                    </div>
                    <div className="w-1/2 flex flex-col">
                      ...
                      <textarea required className="w-full h-40 bg-[#121212] text-green-400/90 p-3 rounded border border-dark-border font-mono text-sm outline-none focus:border-green-600 resize-none custom-scrollbar"
                        value={tc.expectedOutput || tc.expected_output} onChange={(e) => updateTestCase(index, 'expectedOutput', e.target.value)} />
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="mt-8 flex justify-between items-center bg-[#1e1e1e] p-5 rounded-lg border border-dark-border sticky bottom-4 shadow-2xl z-10">
              ...
              <Button onClick={handleSaveChanges} variant="success" className="px-8 shadow-lg shadow-green-900/20" 
                disabled={isSubmitting || testCases.some(tc => !(tc.input || tc.input_data)?.trim() || !(tc.expectedOutput || tc.expected_output)?.trim())}>
